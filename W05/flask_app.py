...
@app.route('/api/temp/t1t2', methods=['GET'])
def api_temp_t1t2():
    #- 取得時間範圍的參數
    ...
    sql_cmd_str = "select * from %s WHERE dtime>'%s'" %(table_name, t1)
    resultProxy=my_db2.execute(sql_cmd_str)
    data = resultProxy.fetchall()
    print('-- data --')
    print(data)
...
def api_temp_post():
    ...
    sql_cmd_str = "insert into %s (dtime, temp) values('%s', '%s')" %(table_name, time2db, temp)
    sql_cmd_str = sql_cmd_str + (" ON DUPLICATE KEY UPDATE temp='%s'" %(temp))
    resultProxy=my_db2.execute(sql_cmd_str)

    return jsonify({'result':'OK', 'log':'insert: [%s,%s]' %(time2db, temp) })


@app.route('/api/temp/form', methods=['POST'])
def api_temp_post_form():
